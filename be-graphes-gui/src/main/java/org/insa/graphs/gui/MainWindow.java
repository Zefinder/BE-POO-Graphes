...
import org.insa.graphs.algorithm.shortestpath.ElectricCarAlgorithm;
...
public class MainWindow extends JFrame {
	...
	private final AlgorithmPanel wccPanel, spPanel, cpPanel, psPanel, elecCarPanel;
	...
	public MainWindow() {
		...
		wccPanel = new AlgorithmPanel(this, WeaklyConnectedComponentsAlgorithm.class, "Weakly-Connected Components",
				new String[] {}, false);
		...
		cpPanel = new AlgorithmPanel(this, CarPoolingAlgorithm.class, "Car-Pooling",
				new String[] { "Origin Car", "Origin Pedestrian", "Destination Car", "Destination Pedestrian" }, true);

		psPanel = new AlgorithmPanel(this, PackageSwitchAlgorithm.class, "Package-Switch Algorithm",
				new String[] { "Oribin A", "Origin B", "Destination A", "Destination B" }, true);

		elecCarPanel = new AlgorithmPanel(this, ElectricCarAlgorithm.class, "Electric Car Shortest-Path",
				new String[] { "Origin", "Destination" }, true);
		
		elecCarPanel.addStartActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				
				// On initialise les paramètres de l'algo en copiant celui du shortest path
				StartActionEvent evt = (StartActionEvent) e;
				ShortestPathData data = new ShortestPathData(graph, evt.getNodes().get(0), evt.getNodes().get(1),
						evt.getArcFilter());
				...
				// On lance l'algo en créant un thread qui dessine sur le Panel
				launchThread(new Runnable() {
		...
		launchThread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					progressBar = null;
					JOptionPane.showMessageDialog(MainWindow.this,
							"<html><p>Unable to read graph from the selected file:</p><p>" + exception.getMessage()
									+ "</p>");
				...
				info += ", " + graph.size() + " nodes, " + graph.getGraphInformation().getArcCount() + " arcs.";
	...
	private JMenuBar createMenuBar(ActionListener openMapActionListener) {
		...
		openPathItem.addActionListener(baf.createBlockingAction(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (chooser.showOpenDialog(MainWindow.this) == JFileChooser.APPROVE_OPTION) {
					try (BinaryPathReader reader = new BinaryPathReader(new DataInputStream(
							new BufferedInputStream(new FileInputStream(chooser.getSelectedFile()))))) {
		...
		// Shortest path
		JMenuItem elecCarItem = new JMenuItem("Electric Car Shortest-Path");
		elecCarItem.addActionListener(baf.createBlockingAction(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				enableAlgorithmPanel(elecCarPanel);
			}
		}));
		...
		graphLockItems.add(elecCarItem);
