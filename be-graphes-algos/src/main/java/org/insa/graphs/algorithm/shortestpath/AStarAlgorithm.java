...
public class AStarAlgorithm extends DijkstraAlgorithm {
	...
	private ArrayList<LabelStar> labelMap;

	private BinaryHeap<Label> priorityQueue;

	public AStarAlgorithm(ShortestPathData data) {
		super(data);
		labelMap = new ArrayList<LabelStar>();
	...
	protected ShortestPathSolution doRun() {
		...
		for (int i = 0; i < nbNodes; i++) {
			...
			labelMap.add(new LabelStar(graph.get(i), false, Double.POSITIVE_INFINITY, null,
					Point.distance(graph.get(i).getPoint(), destination.getPoint())));
		}
